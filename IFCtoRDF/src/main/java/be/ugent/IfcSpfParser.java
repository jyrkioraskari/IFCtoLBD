			...
			for (int i = 0; i < vo.getObjectList().size(); i++) {
				...
				if (o instanceof Character c) {
					...
					if (s.charAt(0) == '#') {
						...
						if (or == null) {
							LOG.error("*ERROR 6*: Reference to non-existing line number in line: #" + vo.getLineNum()
									+ "=" + vo.getFullLineAfterNum());
							continue;
						}
					...
					for (int j = 0; j < tmpList.size(); j++) {
						...
						if (o instanceof Character c) {
							if (c != ',') {
								LOG.error(
										"*ERROR 16*: We found a character that is not a comma. That should not be possible!");
							}
