# IFCtoRDF
IFCtoRDF is a set of reusable Java components that parse IFC-SPF files and convert them into [RDF](https://www.w3.org/standards/techs/rdf#w3c_all) graphs. This library supports the conversion of IFC/SPF files according to the schemas available at https://technical.buildingsmart.org/standards/IFC/IFC-schema-specifications/, namely:
...
Both are identical and include all necessary dependencies to run the code out of the box.
...
```
java -jar ifc2rdf-1.4.7-shaded.jar --baseURI https://www.myownwebspace.be/ --dir path/to/folder/
java -jar ifc2rdf-1.4.7-shaded.jar --dir path/to/folder/
java -jar ifc2rdf-1.4.7-shaded.jar path/to/file.ifc path/to/file.ttl
java -jar ifc2rdf-1.4.7-shaded.jar --baseURI http://www.test.be/ path/to/file.ifc path/to/file.ttl
```

The conversion process can be memory-intensive. 400MB files are fully loaded into memory, often twice, due to the use of the Jena RDF library, not to mention the IFC OWL ontology. It is therefore to reserve a sufficient amount of RAM memory to this Java process. You can do this by explicitly stating the optimal Java heap space to be used with the `-Xmx` and `-Xms` flags. For example, 8GB RAM is associated to this process in the below command.

```
java -Xmx8g -Xms8g -jar ifc2rdf-1.4.7-shaded.jar --baseURI https://www.myownwebspace.be/ --dir path/to/folder/
```

## How to re-use this code in your own Java code project?
This Java code is managed using [Maven](https://maven.apache.org/). If you plan to reuse this code, you are advised to do so through Maven. The code was originally published as a Maven module in Maven Central (https://search.maven.org/artifact/com.github.pipauwel/IFCtoRDF). 
...
Jyrki Oraskari  
Design Computation, RWTH Aachen University
