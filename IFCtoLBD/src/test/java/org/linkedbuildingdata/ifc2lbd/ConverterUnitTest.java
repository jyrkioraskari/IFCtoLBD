...
public class ConverterUnitTest {
	...
	@DisplayName("ifcOWL test 3")
	@Test
	public void testPerformanceBoost3() {
		System.out.println("Start");
		URL ifc_file_url = ClassLoader.getSystemResource("Duplex.ifc");
		try {
			File ifc_file = new File(ifc_file_url.toURI());
			...
			while(iterator.hasNext())
			{
				Statement s = iterator.next();
				if(s.getObject().isResource())
					if(s.getObject().asResource().getLocalName().toString().equals("IfcCartesianPoint"))
					{
						System.out.println(s.getObject().asResource().getLocalName());
						hasGeometry=true;
						break;
			...
			System.err.println("ERROR");
			e.printStackTrace();
			fail("ifcOWL test 3 had an error: " + e.getMessage());
		}
	}

	@DisplayName("ifcOWL test 4")
	@Test
	public void testPerformanceBoost4() {
		System.out.println("Start");
		URL ifc_file_url = ClassLoader.getSystemResource("Duplex.ifc");
		try {
			File ifc_file = new File(ifc_file_url.toURI());
			...
			if(hasGeometry)
				fail("The graph should not have a geometry");
		} catch (Exception e) {
			System.err.println("ERROR");
			e.printStackTrace();
			fail("ifcOWL test 4 had an error: " + e.getMessage());
		}
	}
}
